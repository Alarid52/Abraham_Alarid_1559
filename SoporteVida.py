...
class SoporteVida:
    ...
    def is_alive_cell(self,row,col):
        if self.__grid.get_item(row,col)==1: # return self.__grid.get_item(row,col) == 1
            return True
        else:
            return False
    ...
    def reglas(self):
        ...
        for r in range(self.__rows):
            for c in range(self.__cols):
                ...
                if vivo == True:
                    if vecino == 2 or vecino == 3:      #Regla 1
...
def main():
    ...
    juego = SoporteVida(5,5) #crea Array2D
    juego.configure([[1,2],[2,1],[2,2],[2,3]],10) #aplica configuración
    juego.inicio_juego() #ejecuta el juego
